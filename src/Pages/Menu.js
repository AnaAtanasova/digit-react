...
const MenuCard = (props) => {
  ...
  return (
    <Grid item xs={2} sm={3} md={4} key={product.index}>
      <Collapse in={open}>
        <Alert
          action={
            <IconButton
    ...
    const fetchProducts = async () => {
      ...
      if (!pageIdxsFetched.includes(currentPage)) {
        const resp = await axios.get(
          `http://localhost:3001/products?restaurant_id=${
            location.state.id
          }&&_start=${(currentPage - 1) * 9}&_end=${currentPage * 9}`
        );
    ...
    <ThemeProvider theme={customTheme}>
      <div style={{ margin: "15px 60px 60px 60px" }}>
        <Typography variant="h4" sx={{ mt: 1.5, mb: 2, float: "left" }}>
          Select <b>products</b> to add to your cart
        </Typography>
        {products.length > 9 && (
          <Pagination
            color="error"
